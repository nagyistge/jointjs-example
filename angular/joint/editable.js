	...
	function ($, _, $ui, form, util) {
		...
		function initPaperEvents(joint, graph, paper) {
			...
			function addExtraProperties(cellView, paper) {
				...
				// 7 delete resize elements
				for (var i = 0; i <= lastCellView.$htmlBox.children().length; i++) {
					lastCellView.$htmlBox.children()[ 1 ].remove();
				}
			}
			...
			function doubleClick(cellView) {
				...
				if (cellView.model.attributes &&
					cellView.model.attributes.attrs &&
					cellView.model.attributes.attrs.custom_attrs &&
					cellView.model.attributes.attrs.custom_attrs.isRoot) {
					return;
				}
